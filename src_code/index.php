...
<body>
	<div id="content">
		<div class="login-card">
			<img src="<?= $logo_src ?>"/><br>
			<h1></h1>
			<form method="POST" action="http://172.16.164.3:8002/index.php">
			</form>
...
$agent = urldecode($_POST['userAgent']);
...
if ($_POST['logout_id']) {
	...
	/*
	$country_region = getLocationInfoByIp($clientip);
	$country = $country_region['country'];
	$region = $country_region['region'];

	add parameters $country and $region to portal_allow function
	*/
	captiveportal_logportalauth("unauthenticated", $clientmac, $clientip, "ACCEPT");
	portal_allow($agent,$clientip, $clientmac, "unauthenticated", null, $redirurl);
	...
	if ($timecredit > 0) {  // voucher is valid. Remaining minutes returned
		...
		if (portal_allow($agent,$clientip, $clientmac, $voucher, null, $redirurl, $attr, null, 'voucher', 'voucher') === 2) {
			portal_reply_page($redirurl, "error", "Reuse of identification not allowed.", $clientmac, $clientip);
		} elseif (portal_allow($agent,$clientip, $clientmac, $voucher, null, $redirurl, $attr, null, 'voucher', 'voucher')) {
	...
	if ($auth_result['result']) {
		...
		portal_allow($agent,$clientip, $clientmac, $user, $passwd, $redirurl, $auth_result['attributes'], $pipeno, $auth_result['auth_method'], $context);
